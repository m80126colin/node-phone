var iso3166_data = [
	// if no country is given, default to China. DO NOT move this position
...
module.exports = function(phone, country) {
	...
	// fail to get iso3166 by phone, try get by country
	if (Object.keys(iso3166).length === 0) {
		...
		iso3166 = getISO3166(country); // if no country, default is China
		if (Object.keys(iso3166).length === 0) {
			return result;
		}
	}
	...
	if (country) {
		...
		if (plus_sign) {
			...
			if (iso3166.phone_number_lengths.indexOf(phone.length) !== -1) {
				phone = iso3166.countrycode + phone;
			}
		}
	}
	*/
