// TO_PAT: this is getting really annoying, so I'm disabling it. If
// necessary, we can make it a debug or something.
// console.warn(">>PATM: loading special phone library");
var iso3166_data = [
...
function validate_phone_length_iso3166(phone, iso3166) {
...
function validate_phone_prefix_iso3166(phone, iso3166) {
	phone = phone.replace(new RegExp('^' + iso3166.country_code), '');
...
function validate_phone_iso3166(phone, iso3166) {
	return validate_phone_length_iso3166(phone, iso3166) && validate_phone_prefix_iso3166(phone, iso3166);
}
...
module.exports = function(phone, country, options) {
	options = options || {};
	...
	if (country) {
        ...
        if (options.checkLength != false && !validate_phone_length_iso3166(phone, iso3166)) {
		return [];
        } else if (options.checkIsMobile != false && !validate_phone_iso3166(phone, iso3166)) {
		return [];
        } else {
